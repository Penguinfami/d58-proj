...
def scheduleLinkStateUpdates(link):
    print("New link delay update %s %s" % (link.intf1.name, time.time()))
    randomDelay = random.randint(1, 10)
    randomBandwidth = random.randint(5, 20)
    link.intf1.config(delay='%dms' % randomDelay)
    if link.intf1.name not in updatedParams:
        updatedParams[link.intf1.name] = link.intf1.params
    updatedParams[link.intf1.name]["delay"] = '%dms' % randomDelay
...
def scheduleTopologyStoring(net, filename):
    ...
    scheduler.enter(5, 1, scheduleTopologyStoring, [net, "ongoingnetworkdata.json"])
